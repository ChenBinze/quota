...
public class AddQuotaInfoTest {
    ...
    public void add() {
        ...
        if (StringUtils.equals(applyResponse.getErrorCode(), ErrorEnum.SUCCESS.getErrorCode())) {
            ...
            addRequest.setOperateType(QuotaOperateTypeEnum.ADD.getCode());
            addRequest.setCurrency(CurrencyEnum.CNY.getCode());
            addRequest.setAmount(new BigDecimal("100"));//金额为空、等于0、小于0，大于0
            QuotaOperateResponse addResponse = quotaOperateService.operate(addRequest);
            System.out.println("额度增加结果:"+ addResponse.getErrorCode() + ":" + addResponse.getErrorMessage());
        }
    }
